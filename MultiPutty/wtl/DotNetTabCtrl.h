...
public:
	...
	{
#ifdef _KONSOLE_TAB
    //m_clrSelectedTab = RGB(255, 255, 255);//::GetSysColor(COLOR_WINDOW);
#endif
  }
...
public:
    ...
    COLORREF clrInactive = RGB(212, 212, 212); // lpNMCustomDraw->clrBtnFace;
#ifndef _KONSOLE_TAB
    if(bHighlighted)
#endif
		{
        ...
        dc.FillSolidRect(&rcHighlight, bHighlighted ? lpNMCustomDraw->clrHighlight : clrInactive);
			}
			else
			{
	...
	void DrawItem_ImageAndText(DWORD /*dwStyle*/, LPNMCTCCUSTOMDRAW lpNMCustomDraw, int nIconVerticalCenter, RECT& rcTab, RECT& rcText)
	{
		...
		int nItem = (int)lpNMCustomDraw->nmcd.dwItemSpec;
    //stdlog("bHighlighted = %d bSelected = %d bHot = %d\n", bHighlighted, bSelected, bHot);
		TItem* pItem = this->GetItem(nItem);

#ifdef _KONSOLE_TAB
    bool bMouseDown = m_iClosetem == nItem ? ectcMouseDownL_CloseButton == (m_dwState & ectcMouseDown) : false;
    bool bMouseOver = m_iClosetem == nItem ? ectcMouseOver_CloseButton == (m_dwState & ectcMouseOver) : false;
    bool bHotTrack = m_iClosetem == nItem ? ectcHotTrack_CloseButton == (m_dwState & ectcHotTrack) : false;
    bool bOnTab = bHot || bMouseDown || bMouseOver || bHotTrack;
#else
bool bOnTab = bHot;
#endif
		...
		{
      crPrevious = dc.SetTextColor(lpNMCustomDraw->clrTextSelected);//clrTextInactive
		}
...
#ifdef _KONSOLE_TAB
    RECT rcCloseButton = pItem->GetCloseButtonRect();
    if (bOnTab)
    {
      DrawCloseButton(lpNMCustomDraw, rcCloseButton, bMouseDown, bMouseOver, bHotTrack);
      rcText.right = rcCloseButton.left;
    }
    //stdlog("draw nItem=%d m_iClosetem=%d l=%d r=%d \n", nItem, m_iClosetem, rcCloseButton.left, rcCloseButton.right);
#endif
  ...
  void DrawCloseButton(LPNMCTCCUSTOMDRAW lpNMCustomDraw, RECT rcCloseButton, bool bMouseDown, bool bMouseOver, bool bHotTrack)
  {
    WTL::CDCHandle dc(lpNMCustomDraw->nmcd.hdc);
    ...
    if (bMouseDown)
    {
      if (bMouseOver)
      {
        dc.DrawEdge(&rcCloseButton, BDR_SUNKENOUTER, BF_RECT);
      }
    }
    else if (bHotTrack)
    {
      dc.DrawEdge(&rcCloseButton, BDR_RAISEDINNER, BF_RECT);
    }
	...
	void DoPostPaint(RECT /*rcClient*/, LPNMCTCCUSTOMDRAW lpNMCustomDraw)
	{
		...
		if(CTCS_CLOSEBUTTON == (dwStyle & CTCS_CLOSEBUTTON))
		{
			if( (m_iCurSel >= 0) && ((size_t)m_iCurSel < m_Items.GetCount()) )
			{
				...
				{
          bool bMouseDown = ectcMouseDownL_CloseButton == (m_dwState & ectcMouseDown);
          bool bMouseOver = ectcMouseOver_CloseButton == (m_dwState & ectcMouseOver);
          bool bHotTrack = ectcHotTrack_CloseButton == (m_dwState & ectcHotTrack);
          pT->DrawCloseButton(lpNMCustomDraw, pT->m_rcCloseButton, bMouseDown, bMouseOver, bHotTrack);
				}
			}
		}
...
public:
	...
	{
#ifdef _KONSOLE_TAB
    static int cxMinText = 0;
    static int cxMaxText = 0;
#endif
    //DWORD dwStyle = this->GetStyle();
    ...
    {
      RECT rcMin = { 0 };
      dc.DrawText(_T("WWWWW"), 5, &rcMin, DT_SINGLELINE | DT_CALCRECT);
      cxMinText = rcMin.right - rcMin.left + (2 * m_settings.iMargin) + 15; //15 for close button
      RECT rcMax = { 0 };
      dc.DrawText(_T("WWWWWWWWWWWWWWWWWWWW"), 20, &rcMax, DT_SINGLELINE | DT_CALCRECT);
      cxMaxText = rcMax.right - rcMax.left + (2 * m_settings.iMargin) + 15; //15 for close button
    }
#endif
		...
		HFONT hRestoreNormalFont = NULL;
#ifdef _KONSOLE_TAB
    int cxItemArea = (rcTabItemArea.right - rcTabItemArea.left - xpos);
    int cxItem = cxItemArea / (nCount + 1);
    if (cxItem < cxMinText)
      cxItem = cxMinText;
    if (cxItem > cxMaxText)
      cxItem = cxMaxText;

    int nVisibleItem = cxItemArea / cxItem;
    if (nVisibleItem > 0)
      cxItem = cxItemArea / nVisibleItem;
    SetScrollDelta(cxItem);
    ...
    for (size_t i = 0; i < nCount; ++i)
    {
      ...
      ATLASSERT(pItem != NULL);
      rcItem.left = xpos;
      rcItem.right = rcItem.left + cxItem;
      pItem->SetRect(rcItem);
      xpos += (rcItem.right - rcItem.left);
    }
#else
