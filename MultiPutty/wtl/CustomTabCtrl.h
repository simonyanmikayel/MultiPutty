...
public:
  ...
  RECT GetCloseButtonRect() const
  {
    ...
    if ((m_rcItem.right - m_rcItem.left) < nButtonSizeX)
    {
      ::SetRectEmpty(&m_rcCloseButton);
    }
    else
    {
      m_rcCloseButton = m_rcItem;
...
public:
	...
	LRESULT OnMouseMove(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
	{
      ...
      RECT *prcCloseButton = NULL;
#ifdef _KONSOLE_TAB
      if (nIndex >= 0 && nIndex < (int)m_Items.GetCount())
      {
        prcCloseButton = &(m_Items[nIndex]->GetCloseButtonRect());
        OffsetRect(prcCloseButton, m_iScrollOffset, 0);
        //stdlog("move nIndex=%d l=%d r=%d x=%d y=%d\n", nIndex, prcCloseButton->left, prcCloseButton->right, ptCursor.x, ptCursor.y);
      }
#else
      prcCloseButton = &m_rcCloseButton;
#endif
      if (boolHitNotFound && prcCloseButton && ::PtInRect(prcCloseButton, ptCursor))
			{
        ...
        if (ectcMouseOver_CloseButton != (m_dwState & ectcMouseOver))
				{
					...
					{
            this->InvalidateRect(prcCloseButton);
					}
				}
    ...
    prcCloseButton = &m_rcCloseButton;
#endif
    if (prcCloseButton && ::PtInRect(prcCloseButton, ptCursor))
		{
      m_iClosetem = nIndex;
	...
	LRESULT OnLButtonUp(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
	{
		if(m_hWnd == ::GetCapture())
		{
				...
				ectcMouseOver_CloseButton == (dwState & ectcMouseOver))
			{
				// Close Button
#ifdef _KONSOLE_TAB
                NMCTCITEM nmh = { { m_hWnd, this->GetDlgCtrlID(), CTCN_CLOSE }, m_iClosetem, { ptCursor.x, ptCursor.y }, 0 };
#else
