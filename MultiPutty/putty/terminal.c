...
static void allpos(Terminal *term, pos *top, pos *bottom);
static int sblines(Terminal *term);
static void update_search(Terminal *term, pos top, pos bottom);
#ifdef OPTIMISE_SCROLL
static void scroll_display(Terminal *, int, int, int);
#endif /* OPTIMISE_SCROLL */

static void toppos(Terminal *term, pos *p)
{
    p->y = -sblines(term);
    p->x = 0;
}
static void bottompos(Terminal *term, pos *p)
{
    tree234 *screen = term->screen;
    p->y = find_last_nonempty_line(term, screen);
    p->x = term->cols;
}
...
typedef struct {
    int  text_len; //amount of actual data
    int  text_size; //amount of allocated space
    wchar_t *text;	    // buffer for text 
    int  lines_len; //amount of actual data
    int  lines_size; //amount of allocated space
    int *lines; //lines position

} search_workbuf;

#define search_addchar(b, chr)\
{\
    if (b.text_len >= b.text_size) {\
        b.text_size += 1024;\
        b.text = sresize(b.text, b.text_size, wchar_t);\
    }\
    b.text[b.text_len] = chr;\
    b.text_len++;\
}
#define search_add_line(b, line)\
{\
    if (b.lines_len >= b.lines_size) {\
        b.lines_size += 128;\
        b.lines = sresize(b.lines, b.lines_size, int);\
        }\
...
static unsigned char *compressline(termline *ldata)
{
    ...
    */
    ...
    // time
    {
        unsigned char *n = (unsigned char*)(&ldata->line_time);
        add(b, n[0]);
        add(b, n[1]);
        add(b, n[2]);
        add(b, n[3]);
...
static termline *decompressline(unsigned char *data, int *bytes_used)
{
    ...
    */
    ...
    // time
    unsigned char *n = (unsigned char*)(&line_time);
    n[0] = get(b);
    n[1] = get(b);
    n[2] = get(b);
    n[3] = get(b);
...
void *term_lineptr(Terminal *term, int y)
{
    return lineptr(term, y, __LINE__, FALSE);
}


static int abs_pos(Terminal *term, int y)
{
    return y + sblines(term);
}
...
void term_clrsb(Terminal *term)
{
    ...
    */
    pop_search_buf(count234(term->scrollback));
...
void term_size(Terminal *term, int newrows, int newcols, int newsavelines)
{
    ...
    */
    ...
    pop_search_buf(sblen - newsavelines);
...
static void set_line_time(termline *cline)
{
    if (cline->line_time == 0)
    {
        SYSTEMTIME t;
        GetLocalTime(&t);
        WORD* p = (WORD*)(&cline->line_time);
        p[0] = t.wHour;
        p[1] = t.wMinute;
        p[2] = t.wSecond;
        p[3] = t.wMilliseconds;
...
static void do_paint(Terminal *term, Context ctx, int may_optimise)
{
    ...
    */
    ...
    init_search_for_paint(abs_pos(term, term->disptop), 0);
    ...
    for (i = 0; i < term->rows; i++) {
        ...
        */
        for (j = 0; j < term->cols; j++) {
            ...
            int is_search = is_search_line(abspos, j);
            if (is_search)
                tattr |= (is_search == 1) ? ATTR_SEARCH1 : ATTR_SEARCH2;
                ...
                if ((dirty_run || last_run_dirty) && ccount > 0) {
                    ...
                    if (attr & (TATTR_ACTCURS | TATTR_PASCURS))
                        do_cursor(ctx, start, i, ch, ccount, attr,
                            ldata->lattr);
                    if (first_dirty > i) first_dirty = i;
                    if (last_dirty < i); last_dirty = i;
                }
                start = j;
                ccount = 0;
                attr = tattr;
...
{
    int attr;
    search_workbuf buf = { 0 };
    wchar_t *wtext = get_search_text();

    BOOL bMatchCase = (get_search_settings() & SEARCH_MATCHCASE);
    BOOL bMatchWholeWord = (get_search_settings() & SEARCH_MATCHWHOLEWORD);
    WORD wLine = 0;
    WORD wPos = 0;
    pos cur_top;
    ...
    toppos(term, &cur_top);
    if (poslt(top, cur_top))
        top = cur_top;

    int search_buff_line = abs_pos(term, top.y);
    ...
    while (poslt(top, bottom)) {
        ...
        search_add_line(buf, buf.text_len);
    ...
    if (buf.lines_len)
    {
        search_addchar(buf, 0);
        int offset = 0;
        int pos = 0;
        wchar_t* p = buf.text;
        int line = 0;
        int c = wcslen(wtext); //length of search text
        set_search_text_size(c);
        while (p = find_str(p, wtext, bMatchCase))
        {
            offset = p - buf.text;
            while (buf.lines[line] <= offset && line < buf.lines_len)
            {
                line++;
            }
            pos = offset - buf.lines[line - 1];
            if (!bMatchWholeWord || ((p == buf.text || !isalpha(*(p - 1))) && !isalpha(*(p + c))))
            {
                add_search_pos(search_buff_line + line - 1, pos);
            }
            p += c;
        }
        sfree(buf.text);
...
static void do_search(Terminal *term, wchar_t *wtext, WORD wSearchSettings, WORD cmd)
{
    pos top;
    pos bottom;
    ...
    if (!wtext[0])
    {
        dump_serarch_buf("do_search 0");
        clean_serarch_buf();
        return;
    }

    if (cmd != ID_SEARCH_REFRESH)
    {
        if (get_search_text() != 0)
        {
            if (0 == wcscmp(get_search_text(), wtext) && wSearchSettings == get_search_settings())
            {
                return;
            }
        }
    ...
    dump_serarch_buf("do_search 1");
    clean_serarch_buf();
    set_search_text(wtext);
    set_search_settings(wSearchSettings);
    allpos(term, &top, &bottom);

    update_search(term, top, bottom);
}

LRESULT term_find_text(Terminal *term, wchar_t *text, WORD cmd, WORD wSearchSettings)
{
    do_search(term, text, wSearchSettings, cmd);

    int abs_line = set_cur_pos(cmd);
    if (abs_line != 0xFFFFFFF)
    {
        int line = abs_line - sblines(term);
        //stdlog("line=%d disptop=%d term->rows=%d\n", line, term->disptop, term->rows);
        if (line < term->disptop)
        {
            term_scroll(term, +1, abs_line - term->rows / 2);
        }
        else if (line >= term->disptop + term->rows)
        {
            term_scroll(term, +1, abs_line - term->rows / 2);
        }
    }
...
static void clipme(Terminal *term, pos top, pos bottom, int rect, int desel, BOOL with_timestamp, char* szFile)
{
    clip_workbuf buf;
    int old_top_x;
    int attr;
        ...
        */
        ...
        if (with_timestamp)
        {
            ...
            if (ldata->line_time)
            {
                ...
                sprintf(timestamp, "%02d:%02d:%02d.%03d ", p[0] % 100, p[1] % 100, p[2] % 100, p[3] % 1000);
            }
            else
            {
                sprintf(timestamp, "             ");
    ...
    if (szFile)
        save_clip(term->frontend, buf.textbuf, buf.attrbuf, buf.bufpos, szFile);
    else
...
int term_last_nonempty_line(Terminal *term)
{
    tree234 *screen = term->screen;
    return find_last_nonempty_line(term, screen);
}

void term_copy(Terminal *term, BOOL with_timestamp)
{
    if (term->selstate == SELECTED)
        clipme(term, term->selstart, term->selend,
        (term->seltype == RECTANGULAR), FALSE, with_timestamp, NULL);
}

void term_copyall(Terminal *term, BOOL with_timestamp, char* szFile)
{
    pos top;
    pos bottom;
    allpos(term, &top, &bottom);
    clipme(term, top, bottom, 0, FALSE, with_timestamp, szFile);
}
    ...
    */
    ...
    if (bcooked == MBT_SELECT && a == MA_CLICK) {
        ...
        if (term->selstate == DRAGGING) {
            /*
            * We've completed a selection
            */
	    clipme(term, term->selstart, term->selend,
		   (term->seltype == RECTANGULAR), FALSE, FALSE, NULL);
            term->selstate = SELECTED;
	} else
            term->selstate = NO_SELECTION;
...
void term_selectall(Terminal *term)
{
    allpos(term, &term->selstart, &term->selend);
    term->selstate = SELECTED;
    term_update(term);
}
