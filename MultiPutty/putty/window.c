...
int PuttyWinMain(HINSTANCE inst, char* cmdline)
{
    dll_hijacking_protection();
    g_cmdline = strdup(cmdline);
	hinst = inst;
	hwnd = NULL;
	flags = FLAG_VERBOSE | FLAG_INTERACTIVE;

    //char msg[50];
    //sprintf(msg, "Putty id = %d", GetCurrentProcessId());
    //MessageBox(NULL, msg, "Putty", MB_OK | MB_ICONEXCLAMATION);
    ...
    fatal_error("Windows refuses to report a version");
  ...
  wndclass.hIcon = NULL;// LoadIcon(inst, MAKEINTRESOURCE(IDI_PUTTY_MAINICON));
  wndclass.hCursor = LoadCursor(NULL, IDC_IBEAM);
	wndclass.hbrBackground = NULL;
	wndclass.lpszMenuName = NULL;
  wndclass.lpszClassName = uappname;
    ...
    */
	...
	if (*p == '@') {
            ...
            */
            ...
            if (sscanf(p + 1, "%p:%u:%u", &filemap, &cpsize, &hwndHost) == 3 &&
                (cp = MapViewOfFile(filemap, FILE_MAP_READ,
                0, 0, cpsize)) != NULL) {
                conf_deserialise(conf, cp, cpsize);
                multiPuttyCmd = strchr(p + 1, '@');
                if (!multiPuttyCmd)
                    multiPuttyCmd = "";
                else
                    multiPuttyCmd++;
                conf_set_str(conf, CONF_multi_putty_cmd, multiPuttyCmd);
                UnmapViewOfFile(cp);
                CloseHandle(filemap);
            }
            else if (!do_config()) {
...
int PuttyStart()
{
...

    {
      ...
      */
      ...
      hAccelTable = LoadAcceleratorsW(hinst, MAKEINTRESOURCEW(IDR_MAINFRAME));

      //~~int winmode = WS_OVERLAPPEDWINDOW | WS_VSCROLL;
      int winmode = WS_CLIPCHILDREN |WS_CHILD;
      int exwinmode = 0;// WS_EX_TOOLWINDOW;
...

      {
        ...
        if (!hwnd)
        {
          char *error = dupprintf("unable to create window: %s",
  ...
  */
  ...
  hwndHostMain = (HWND)SendMessage(hwndHost, WM_MENUCOMMAND, MSG_PUTTY_HWND, (LPARAM)hwnd);
  /*
  ...
  */
  ...
  ////style |= WS_MAXIMIZE;
  //style &= ~(WS_CAPTION | WS_BORDER | WS_THICKFRAME | WS_OVERLAPPED);
  ...
  term_set_focus(term, TRUE);//GetForegroundWindow() == hwnd
  ...
  UpdateWindow(hwnd);
	...
	int nhandles, n;
        ...
        if (toplevel_callback_pending() ||
            ...
            term_set_focus(term, GetFocus() == hwnd);//GetFocus() == hwndHost
    ...
    { 
      if (TranslateAcceleratorW(hwndHostMain, hAccelTable, &msg))
        continue;
    }
    //stdlog("msg.message = %x\n", msg.message);
  ...
  finished:
    CLOSING = 1;
    fatal_error("Session Ended.");
    cleanup_exit(msg.wParam);	       /* this doesn't return... */
    return msg.wParam;		       /* ... but optimiser doesn't know */
}

static unsigned long PuttyNewPutty(WPARAM wParam, LPARAM lParam, HWND hWndParent, const char* command)
{
    char b[2048];
    char c[512], *cl;
    int freecl = FALSE;
    ...
    if (wParam == IDM_DUPSESS) {
        ...
        */
        ...
        //sprintf(c, "putty &%p:%u:%u", filemap, (unsigned)size, hWndParent);
        sprintf(c, "putty &%p:%u:%u@%s", filemap, (unsigned)size, hWndParent, command);
        cl = c;
    }
...
unsigned long PuttyDuplicate(HWND hWndParent, const char* command)
{
    return PuttyNewPutty(IDM_DUPSESS, 0, hWndParent, command);
}

void PuttyCleanup(void)
{
    /*
    * Clean up.
    */
...
void connection_fatal(void *frontend, const char *fmt, ...)
{
    ...
    fatal_error(stuff);
    //~~ MessageBox(hwnd, stuff, morestuff, MB_ICONERROR | MB_OK | MB_TASKMODAL);
    sfree(stuff);
...
#define TIMMING_WIDTH (13 * font_width)
void nc_paint()
{
	DWORD fg = RGB(32, 32, 32);
	DWORD bg = RGB(128, 128, 128);
	HBRUSH nc_color, term_color;

	if (!conf_get_int(conf, CONF_showtimestamp))
    return;

    nc_color = CreateSolidBrush(bg);
    term_color = CreateSolidBrush(colours[ATTR_DEFBG >> ATTR_BGSHIFT]);
    ...
    char tmp[50];
    int i = 0;
    int last_nonempty = term_last_nonempty_line(term) + 1;
    RECT rc;
    rc.left = 0;
    rc.right = TIMMING_WIDTH - font_width;
    for (i = 0; i < last_nonempty; i++)
    {
        termline *ldata = term_lineptr(term, i + term->disptop);

        if (ldata->line_time)
        {
            WORD* p = (WORD*)(&ldata->line_time);
            int c = sprintf(tmp, "%02d:%02d:%02d.%03d", p[0], p[1], p[2], p[3]);
            TextOut(hdc, 0, (i * font_height) + offset_height, tmp, c);
        }
        else
        {
          rc.top = i*font_height;
          rc.bottom = rc.top + font_height + 1;
          FillRect(hdc, &rc, nc_color);
        }

        term_unlineptr(ldata);
    }
    ...
    rc.left = TIMMING_WIDTH - font_width;
    rc.right = TIMMING_WIDTH;
    FillRect(hdc, &rc, term_color);
				...
				WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COPYDATA:
    {
      ...
      {
          wchar_t* text = pCDS->cbData > 2 ? (wchar_t*)pCDS->lpData : L"";
          return term_find_text(term, text, LOWORD(pCDS->dwData), HIWORD(pCDS->dwData));
        break;
      }
      case ID_EDIT_COPY_TO_FILE:
      {
        char* szFile = pCDS->cbData ? (char*)pCDS->lpData : "";
        if (szFile[0])
        {
          char str[2 * MAX_PATH];
          sprintf(str, "%s", szFile);
          term_copyall(term, FALSE, str);
          sprintf(str, "%s.txt", szFile);
          term_copyall(term, TRUE, str);
        }
        else
        {
          term_copyall(term, TRUE, szFile);
        }
        break;
      }
      }
    ...
    case WM_MENUCOMMAND:
    {
      ...
      case MSG_PUTTY_COMMAND:
      {
        ...
        case ID_SCROLL_BOTTOM:
            ...
            break;
        case ID_EDIT_COPY:
          term_copy(term, FALSE);
          break;
        case ID_EDIT_COPY_WITH_TIMESTAMP:
          term_copy(term, TRUE);
          break;
        case ID_EDIT_COPY_ALL:
          term_copyall(term, FALSE, NULL);
          break;          
        case ID_EDIT_COPY_ALL_WITH_TIMESTAMP:
          term_copyall(term, TRUE, NULL);
          break;
        ...
        case ID_EDIT_RESET_TERMINAL:
            ...
            if (lParam != ID_EDIT_RESET_TERMINAL) {
                term_clrsb(term);
            }
            break;
        }
      ...
      case WM_CLOSE:
	{
      ...
      int warn_on_close = conf_get_int(conf, CONF_warn_on_close);
      warn_on_close = 0; //disable worning
      if (session_closed || !warn_on_close ||
		MessageBox(hwnd,
	...
	break;
      case WM_SYSCOMMAND:
        //SendMessage(hwndHostMain, message, wParam, lParam);
        //return 0;
        //break;
      case WM_COMMAND:
  ...
  case IDM_SHOWLOG:
	  ...
	  case IDM_RECONF:
		...
		reconfig_result =
		    do_reconfig(hwndHostMain, back ? back->cfg_info(backhandle) : 0);
		reconfiguring = FALSE;
		if (!reconfig_result) {
                    conf_free(prev_conf);
		    break;
    ...
    if (conf_get_int(prev_conf, CONF_showtimestamp) != 
      conf_get_int(conf, CONF_showtimestamp))
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
    ...
    PostMessage(hwndHost, WM_MENUCOMMAND, MSG_PUTTY_CONTEXT_MENU, MAKELONG(0, 0));
    /*~~~
    if ((wParam & MK_CONTROL) || (conf_get_int(conf, CONF_mouse_is_xterm) == 2))
    {
      POINT cursorpos;
  ...
  */
	...
	return 0;
      case WM_SETFOCUS:
        if (!term)
          break;
        //ShowWindow(hwndHostMain, SW_SHOW);
        //SetForegroundWindow(hwndHostMain);
  ...
  break;
      case WM_KILLFOCUS:
        ...
        //if (GetFocus() != hwndHost && GetFocus() != hwnd)
#endif
        {
          show_mouseptr(1);
...
#endif
	//~~if (wParam == SIZE_MINIMIZED) SetWindowText(hwnd, conf_get_int(conf, CONF_win_name_always) ? window_name : icon_name);
		      ...
		      unsigned long attr, int lattr)
{
    ...
    if (attr & ATTR_SEARCH1) {
      SetTextColor(hdc, RGB(220, 128, 92));
      SetBkColor(hdc, RGB(120, 0, 0));
    }
    else if (attr & ATTR_SEARCH2) {
      SetTextColor(hdc, RGB(184, 184, 92));
      SetBkColor(hdc, RGB(120, 120, 42));
    }
    else {
      SetTextColor(hdc, fg);
      SetBkColor(hdc, bg);
...
void set_sbar(void *frontend, int total, int start, int page)
{
    ...
    si.cbSize = sizeof(si);
    si.fMask = SIF_ALL | SIF_DISABLENOSCROLL;
    //si.fMask = SIF_PAGE | SIF_TRACKPOS | SIF_DISABLENOSCROLL; // (SIF_RANGE | SIF_PAGE | SIF_POS | SIF_TRACKPOS | SIF_DISABLENOSCROLL)
    si.nMin = 0;
    si.nMax = total - 1;
    si.nPage = page;
    si.nPos = start;
...
void save_clip(void *frontend, wchar_t * data, int *attr, int len, char* szFile)
{
  int len2;
  char *data2, *p;
  FILE *fp = NULL;
  ...
  data2 = snewn(len2 + 1, char);
  WideCharToMultiByte(CP_ACP, 0, data, len, data2, len2, NULL, NULL);
  data2[len2] = 0;
  p = data2;
  while (*p) {
    if (*p == '\r' && *(p + 1) == '\n')
...
void set_config_name(const char* name)
{
    if (name == NULL)
        name = "";

    sfree(conf_name);
    conf_name = strdup(name);
}
...
char* get_multi_session(int index)
{
  char str_key[16];
  sprintf(str_key, "%04d", index);
...
void print_buf(const char *data, int len)
{
  ...
  for (i = 0; i < len; i++)
  {
    ...
    {
      if (i == 0 || data[i - 1] != '\r')
        term_data(term, 0, "\r", 1);
    }
    term_data(term, 0, data + i, 1);
  }
}
