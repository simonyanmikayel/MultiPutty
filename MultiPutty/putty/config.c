...
struct multi_putty_data {
    union control *titlebox, *commandbox, *listbox;
    int len;
    int* id;
    char* str;
};

static void multi_putty_refresh_tab_list(Conf *conf, union control *ctrl, void *dlg)
{
  int count = dlg_listbox_getcount(ctrl, dlg);
  conf_clean_str_strs(conf, CONF_multiputty);
  ...
  //char title[max_multiputty_tab_title_size];
  //char command[max_multiputty_tab_command_size];
  //sscanf(text, "%s\t%s", title, command);
  for (int i = 0; i < count; i++)
  {
    dlg_listbox_gettext(ctrl, dlg, i, sz_val);
    sprintf(str_key, "%04d", i);
    conf_set_str_str(conf, CONF_multiputty, str_key, sz_val);
  }
}

static void multi_putty_handler(union control *ctrl, void *dlg,
    void *data, int event)
{
    // titlebox, *commandbox, *listbox;
    Conf *conf = (Conf *)data;
    struct multi_putty_data *mpd =
        (struct multi_putty_data *)ctrl->generic.context.p;
    ...
    if (ctrl == mpd->listbox) {
      if (event == EVENT_REFRESH) {
        ...
        for (int i = 0;; i++) {
          sprintf(str_key, "%04d", i);
          sz_val = conf_get_str_str_opt(conf, CONF_multiputty, str_key);
          if (!sz_val)
            break;
          dlg_listbox_add(ctrl, dlg, sz_val);
        };
        dlg_update_done(ctrl, dlg);
      }
      else if (event == EVENT_VALCHANGE) {
        multi_putty_refresh_tab_list(conf, ctrl, dlg);
      }
      else if (event == EVENT_VALADD) {
        char *title, *command, *sz_val;
        title = dlg_editbox_get(mpd->titlebox, dlg);
        if (!*title) {
          sfree(title);
          dlg_beep(dlg); 
          return;
        }
        command = dlg_editbox_get(mpd->commandbox, dlg);

        sz_val = dupprintf("%s\t%s", title, command);
        dlg_listbox_add(ctrl, dlg, sz_val);
        multi_putty_refresh_tab_list(conf, ctrl, dlg);

        dlg_editbox_set(mpd->titlebox, dlg, "");
        dlg_editbox_set(mpd->commandbox, dlg, "");
        sfree(sz_val);
        sfree(title);
        sfree(command);
      }
      else if (event == EVENT_VALREMOVE) {
        int i = dlg_listbox_index(mpd->listbox, dlg);
        if (i < 0) {
          dlg_beep(dlg);
        }
        else {
          dlg_listbox_del(ctrl, dlg, i);
          multi_putty_refresh_tab_list(conf, ctrl, dlg);
          //dlg_refresh(mpd->listbox, dlg);
        }
      }
      else if (event == EVENT_VALEDIT) {
        int i = dlg_listbox_index(mpd->listbox, dlg);
        if (i < 0) {
          dlg_beep(dlg);
        }
          ...
          sz_val = dupprintf("%s\t%s", title, command);
          dlg_listbox_del(ctrl, dlg, i);
          dlg_listbox_insert(ctrl, dlg, i, sz_val);
          multi_putty_refresh_tab_list(conf, ctrl, dlg);
        ...
        if (i >= 0) {
          ...
          if (!command)
            ...
            *(command++) = 0;
          dlg_editbox_set(mpd->titlebox, dlg, title);
          dlg_editbox_set(mpd->commandbox, dlg, command);
        }
		      ...
		      int protocol, int protcfginfo)
{
    ...
    ssd->shortcutbutton = ctrl_pushbutton(s, "Shortcut", 'u',
      HELPCTX(session_saved),
      sessionsaver_handler, P(ssd));
    ssd->shortcutbutton->generic.column = 1;
    if (!midsession) {
    ...
    /*
    * The Session/Multi panel.
    */
        ctrl_settitle(b, "Session/MultiPutty", "MultiPutty settings");
        mpd = (struct multi_putty_data*) ctrl_alloc(b, sizeof(struct multi_putty_data));        

        s = ctrl_getset(b, "Session/MultiPutty", "view",
          "View");
        ctrl_checkbox(s, "Show time stamp", 't',
          HELPCTX(telnet_environ),
          conf_checkbox_handler, I(CONF_showtimestamp));

        if (!midsession) {
          s = ctrl_getset(b, "Session/MultiPutty", "tabs", "Tabs");
        mpd->titlebox = ctrl_editbox(s, "Title", NO_SHORTCUT, 80,
            HELPCTX(telnet_environ),
            multi_putty_handler, P(mpd), P(NULL));
        mpd->commandbox = ctrl_editbox(s, "Command", NO_SHORTCUT, 80,
            HELPCTX(telnet_environ),
            multi_putty_handler, P(mpd), P(NULL));

        //ctrl_columns(s, 2, 80, 20);

        mpd->listbox = ctrl_listbox(s, NULL, NO_SHORTCUT,
            HELPCTX(telnet_environ),
            multi_putty_handler, P(mpd));
        mpd->listbox->listbox.height = 16;
        mpd->listbox->listbox.ncols = 2;
        mpd->listbox->listbox.percentages = snewn(2, int);
        mpd->listbox->listbox.percentages[0] = 30;
        mpd->listbox->listbox.percentages[1] = 70;
        mpd->listbox->listbox.draglist = 2;
