                                ...
                                WPARAM wParam, LPARAM lParam)
{
	...
	}
	{
	    ...
	    SendDlgItemMessage(hwnd, IDN_PUTTY_LIST, LB_SETTABSTOPS, 2,
			       (LPARAM) tabs);
	}
	for (i = 0; i < nevents; i++)
	    SendDlgItemMessage(hwnd, IDN_PUTTY_LIST, LB_ADDSTRING,
			       0, (LPARAM) events[i]);
	return 1;
      case WM_COMMAND:
	switch (LOWORD(wParam)) {
	  ...
	  case IDN_PUTTY_COPY:
		...
		int selcount;
		int *selitems;
		selcount = SendDlgItemMessage(hwnd, IDN_PUTTY_LIST,
					      LB_GETSELCOUNT, 0, 0);
		...
		if (selitems) {
		    int count = SendDlgItemMessage(hwnd, IDN_PUTTY_LIST,
						   LB_GETSELITEMS,
						   selcount,
						   (LPARAM) selitems);
		    int i;
		    ...
		    for (i = 0; i < nevents; i++)
			SendDlgItemMessage(hwnd, IDN_PUTTY_LIST, LB_SETSEL,
					   FALSE, i);
		}
	    }
	    return 0;
                                  ...
                                  WPARAM wParam, LPARAM lParam)
{
	...
	switch (LOWORD(wParam)) {
	  ...
	  case IDCANCEL:
	    EndDialog(hwnd, TRUE);
	    return 0;
	  case IDA_PUTTY_LICENCE:
	    EnableWindow(hwnd, 0);
	    DialogBox(hinst, MAKEINTRESOURCE(IDD_PUTTY_LICENCEBOX),
		      hwnd, LicenceProc);
	    EnableWindow(hwnd, 1);
                                           ...
                                           WPARAM wParam, LPARAM lParam)
{
	...
	SetWindowLongPtr(hwnd, GWLP_USERDATA, 0);
        if (has_help())
            ...
            HWND item = GetDlgItem(hwnd, IDC_PUTTY_HELPBTN);
            if (item)
                DestroyWindow(item);
        }
	SendMessage(hwnd, WM_SETICON, (WPARAM) ICON_BIG,
		    (LPARAM) LoadIcon(hinst, MAKEINTRESOURCE(IDI_PUTTY_CFGICON)));
	/*
	 * Centre the window.
	 */
	{			       /* centre the window */
...
{
    EnableWindow(hwnd, 0);
    DialogBox(hinst, MAKEINTRESOURCE(IDD_PUTTY_ABOUTBOX), hwnd, AboutProc);
    EnableWindow(hwnd, 1);
    SetActiveWindow(hwnd);
}
...
void defuse_showwindow(void)
{
    ...
    {
	HWND hwnd;
	hwnd = CreateDialog(hinst, MAKEINTRESOURCE(IDD_PUTTY_ABOUTBOX),
			    NULL, NullDlgProc);
...
int do_config(void)
{
    ...
    ret =
	SaneDialogBox(hinst, MAKEINTRESOURCE(IDD_PUTTY_MAINBOX), NULL,
		  GenericMainDlgProc);
...
int do_reconfig(HWND hwnd, int protcfginfo)
{
    ...
    ret = SaneDialogBox(hinst, MAKEINTRESOURCE(IDD_PUTTY_MAINBOX), NULL,
		  GenericMainDlgProc);
...
void logevent(void *frontend, const char *string)
{
    ...
    if (logbox) {
	int count;
	SendDlgItemMessage(logbox, IDN_PUTTY_LIST, LB_ADDSTRING,
			   0, (LPARAM) events[nevents]);
	count = SendDlgItemMessage(logbox, IDN_PUTTY_LIST, LB_GETCOUNT, 0, 0);
	SendDlgItemMessage(logbox, IDN_PUTTY_LIST, LB_SETTOPINDEX, count - 1, 0);
    }
    nevents++;
}
...
{
    if (!logbox) {
	logbox = CreateDialog(hinst, MAKEINTRESOURCE(IDD_PUTTY_LOGBOX),
			      hwnd, LogProc);
