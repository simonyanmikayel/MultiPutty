...
static char* get_initial_command(Ssh ssh)
{
    char* cmd = conf_get_str(ssh->conf, CONF_multi_putty_cmd);
    if (!(*cmd))
			       ...
			       struct Packet *pktin)
{
     ...
     */
    {
	char *cmd = get_initial_command(ssh);
			     ...
			     struct Packet *pktin)
{
	...
	while (1) {
        ...
        if (first_auth)
        {
            ...
            ret = get_userpass_input(s->cur_prompt, password, password ? strlen(password) : 0);
        }
        else
        {
		    ret = get_userpass_input(s->cur_prompt, NULL, 0);
