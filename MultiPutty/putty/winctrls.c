...
void prefslist(struct prefslist *hdl, struct ctlpos *cp, int lines,
  char *stext, int sid, int listid, int upbid, int dnbid, int addbid, int rmbid, int edbid)
{
    ...
    int totalheight, buttonpos, butttop;
    ...
    for (i=0; i<3; i++) {
          ...
          case 1:
            ...
            r.top = cp->ypos; r.bottom = listheight;
            {
                ...
                ctl = doctl(cp, r, "LISTBOX",
                            WS_CHILD | WS_VISIBLE | WS_TABSTOP |
                WS_VSCROLL | LBS_HASSTRINGS | LBS_USETABSTOPS | (addbid != -1 ? LBS_NOTIFY : 0),
                            WS_EX_CLIENTEDGE,
                            "", listid);
		MakeDragList(ctl);
            }
	     ...
	     * prefslist on a panel */
            ...
            if (addbid != -1) {
              ...
              r.top = butttop;
              r.bottom = PUSHBTNHEIGHT;
              butttop += PUSHBTNHEIGHT + GAPBETWEEN;
              doctl(cp, r, "BUTTON",
		     ...
		     WPARAM wParam, LPARAM lParam)
{
    ...
    if (is_dlmsg) {
        ...
        else if (hdl->addbid != -1 && LOWORD(wParam) == hdl->addbid && ((HIWORD(wParam) == BN_CLICKED) || (HIWORD(wParam) == BN_DOUBLECLICKED))) {
          ret |= 4;
        }
        else if (hdl->rmbid != -1 && LOWORD(wParam) == hdl->rmbid && ((HIWORD(wParam) == BN_CLICKED) || (HIWORD(wParam) == BN_DOUBLECLICKED))) {
          ret |= 8;
        }
        else if (hdl->edbid != -1 && LOWORD(wParam) == hdl->edbid && ((HIWORD(wParam) == BN_CLICKED) || (HIWORD(wParam) == BN_DOUBLECLICKED))) {
          ret |= 16;
        }
		    ...
		    struct ctlpos *cp, struct controlset *s, int *id)
{
	...
	switch (ctrl->generic.type) {
	  ...
	  case CTRL_LISTBOX:
		...
		num_ids = 4;
    BOOL isAddRm = ctrl->listbox.draglist == 2;
    if (isAddRm)
      num_ids += 3;
		prefslist(data, &pos, ctrl->listbox.height, escaped,
              base_id, base_id + 1, base_id + 2, base_id + 3, isAddRm ? base_id + 4 : -1, isAddRm ? base_id + 5 : -1, isAddRm ? base_id + 6 : -1);
    if (isAddRm) {
      shortcuts[nshortcuts++] = 0;   /* Add */
      shortcuts[nshortcuts++] = 0;   /* Remove */
    }
			   ...
			   WPARAM wParam, LPARAM lParam)
{
	...
	if (ctrl->listbox.draglist) {
		...
		ctrl->generic.handler(ctrl, dp, dp->data, EVENT_VALCHANGE);
      if (pret & 4)
        ctrl->generic.handler(ctrl, dp, dp->data, EVENT_VALADD);
      if (pret & 8)
        ctrl->generic.handler(ctrl, dp, dp->data, EVENT_VALREMOVE);
      if (pret & 16)
        ctrl->generic.handler(ctrl, dp, dp->data, EVENT_VALEDIT);
...
void dlg_listbox_insert(union control *ctrl, void *dlg, int index, char const *text)
{
  struct dlgparam *dp = (struct dlgparam *)dlg;
  struct winctrl *c = dlg_findbyctrl(dp, ctrl);
  int msg;
  ...
  SendDlgItemMessage(dp->hwnd, c->base_id + 1, msg, index, (LPARAM)text);
}
...
int dlg_listbox_getcount(union control *ctrl, void *dlg)
{
  ...
  msg = LB_GETCOUNT;
  return SendDlgItemMessage(dp->hwnd, c->base_id + 1, msg, 0, 0);
}

int dlg_listbox_gettext(union control *ctrl, void *dlg, int index, char* text)
{
  struct dlgparam *dp = (struct dlgparam *)dlg;
  struct winctrl *c = dlg_findbyctrl(dp, ctrl);
  int msg;
  assert(c && c->ctrl->generic.type == CTRL_LISTBOX);
  msg = LB_GETTEXT;
  return SendDlgItemMessage(dp->hwnd, c->base_id + 1, msg, index, (LPARAM)text);
